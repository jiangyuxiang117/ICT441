...
document.getElementById('myForm').addEventListener('submit', function(e) {  
	...
	inputs.forEach(function(input) {  
		if (input.value === '') {  
			isEmpty = true;  
			input.parentNode.querySelector('.error').style.display = 'block'; // 显示错误消息  
		} else {  
			input.parentNode.querySelector('.error').style.display = 'none'; // 隐藏错误消息  
		}  
	});  
	if (isEmpty) { // 如果任何一个输入字段为空，阻止表单提交  
		e.preventDefault();  
	}  
});  